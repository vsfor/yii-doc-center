...
class User extends UserIdentity
{
    ...
    public function rules()
    {
        return [
            ...
            ['username', 'required'],
            $this->usernameLengthRule(),
            $this->usernameSpamRule(),
            ['username', 'unique', 
    ...
    private function usernameLengthRule()
    {
        $min = 6;
        if (Yii::$app->getUser()->getIsGuest()) {
            if ((!$this->getIsNewRecord()) && $this->username && strlen($this->username) < 6) {
                $min = 4;
            }
        } else {
            $username = Yii::$app->getUser()->getIdentity()->username;
            $spamArray = explode('|',Yii::$app->params['user.spamNames']);
            if (in_array($username, $spamArray)) {
                $min = 4;
            }
        }
        return ['username', 'string', 'min' => $min, 'max' => 64,'message'=>"username length should between $min and 64"];
    }

    private function usernameSpamRule()
    {
        $pattern = '/\b('.Yii::$app->params['user.spamNames'].')\b/i';
        $spamArray = explode('|',Yii::$app->params['user.spamNames']);
        if (Yii::$app->getUser()->getIsGuest()) {
            if ((!$this->getIsNewRecord()) && $this->username && in_array($this->username,$spamArray)) {
                $pattern = '/\b(shit|fuck)\b/i';
            }
        } else {
            $username = Yii::$app->getUser()->getIdentity()->username;
            if (in_array($username, $spamArray)) {
        ...
        return ['username', 'match',  'not' => true,
        'pattern' => $pattern,
