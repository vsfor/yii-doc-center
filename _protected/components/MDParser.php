...
class MDParser
{
     ...
     * @return string
     */
    private function parseEmoji($str)
    {
        $matches = [];
        preg_match_all("/:([a-zA-Z]{1})([a-zA-Z0-9\-]+):/i", $str, $matches);
        ...
        $emojiBasePath = "http://www.webpagefx.com/tools/emoji-cheat-sheet/graphics/emojis/";
        $emojiExt = ".png";
        $twemojiBasePath = "http://twemoji.maxcdn.com/36x36/";
        $twemojiExt  = ".png";

        foreach ($matches as $match) {
            if (strpos($match, ':tw-') === 0) {
                $t = '<img src="'.$twemojiBasePath . str_replace('tw-','',trim($match, ':')) . $twemojiExt .'" class="emoji twemoji jeenemoji" />';
            } elseif (strpos($match, ':fa-') === 0) {
                $t = '<i class="fa '.trim($match, ':').' fa-emoji jeenemoji"></i>';
            } elseif (strpos($match, ':editormd-') === 0) {
                $t = '';//'<i class="'.trim($match, ':').' jeenemoji"></i>';
            } else {
                $t = '<img class="emoji jeenemoji" src="'.$emojiBasePath. trim($match, ':') .$emojiExt.'" />';
            }
            $str = str_replace($match, $t, $str);
        }
        return $str;
    }
